...
opticalToRadarTimeShiftSec = 0.8;
...
%        %see if we should put this in the strip
%        if yUp > 0 && yDown < size(SBR,1)
%            %SBRstripsX(i,j) = mean(SBRmaskedAll(max([yDown 1]):min([yUp size(SBR,1)]),i));
%            SBRstripsX(i,j) = mean(SBR(max([yDown 1]):min([yUp size(SBR,1)]),j,i));
%        end
%        if xUp > 0 && xDown < size(SBR,1)
%            %SBRstripsY(i,j) = mean(SBRmaskedAll(max([xDown 1]):min([xUp size(SBR,1)]),i));
%            SBRstripsY(i,j) = mean(SBR(j,max([xDown 1]):min([xUp size(SBR,1)]),i));
%        end
   end % end of going through each index to get the SBR strip
   %look at the SBR strips for the i'th image and find the longest streak
   %create an array to hold possible meteors [length,start,end]
    possibleMeteorsX = [[],[],[]];
    possibleMeteorsY = [[],[],[]];
    possibleMeteors = [[],[],[]];
    j = 1;
    while j < size(SBR,1)
        ...
        if SBRstrips(i,j) > 0
            ...
            %threshold
            validPoints = find(SBRstrips(i,j+1:min([j+1+allowedBlankPoints size(SBR,1)])) > SBREndPointThresh);
            if numel(validPoints) > 0
                ...
                possibleMeteors = [possibleMeteors; potentialMeteorEndIndex-potentialMeteorStartIndex+1 potentialMeteorStartIndex potentialMeteorEndIndex];
                %increment j
...
%             %see if there is another point next to it that is over the SBR
%             %threshold
%             validPoints = find(SBRstripsX(i,j+1:min([j+1+allowedBlankPoints size(SBR,1)])) > SBREndPointThresh);
%             if numel(validPoints) > 0
%                 %this is a potential meteor
%                 potentialMeteorStartIndexX = j;
%                 potentialMeteorEndIndexX = j+validPoints(end);
%                 %find the end point of the potential meteor
%                 while potentialMeteorEndIndexX <= size(SBR,1) && numel(validPoints) > 0
%                     validPoints = find(SBRstripsX(i,potentialMeteorEndIndexX+1:min([potentialMeteorEndIndexX+1+allowedBlankPoints size(SBR,1)])) > 0);
%                     if numel(validPoints) > 0
%                         potentialMeteorEndIndexX = validPoints(end)+potentialMeteorEndIndexX;
%                     end
%                 end
%                 %save the potential meteor
%                 possibleMeteorsX = [possibleMeteorsX; potentialMeteorEndIndexX-potentialMeteorStartIndexX+1 potentialMeteorStartIndexX potentialMeteorEndIndexX];
%                 %increment j
%                 j = potentialMeteorEndIndexX+1;
%             else
%                 j = j+1;
%             end
%         else
...
%             %see if there is another point next to it that is over the SBR
%             %threshold
%             validPoints = find(SBRstripsY(i,j+1:min([j+1+allowedBlankPoints size(SBR,1)])) > SBREndPointThresh);
%             if numel(validPoints) > 0
%                 %this is a potential meteor
%                 potentialMeteorStartIndexY = j;
%                 potentialMeteorEndIndexY = j+validPoints(end);
%                 %find the end point of the potential meteor
%                 while potentialMeteorEndIndexY <= size(SBR,1) && numel(validPoints) > 0
%                     validPoints = find(SBRstripsY(i,potentialMeteorEndIndexX+1:min([potentialMeteorEndIndexY+1+allowedBlankPoints size(SBR,1)])) > 0);
%                     if numel(validPoints) > 0
%                         potentialMeteorEndIndexY = validPoints(end)+potentialMeteorEndIndexY;
%                     end
%                 end
%                 %save the potential meteor
%                 possibleMeteorsY = [possibleMeteorsY; potentialMeteorEndIndexY-potentialMeteorStartIndexY+1 potentialMeteorStartIndexY potentialMeteorEndIndexY];
%                 %increment j
%                 j = potentialMeteorEndIndexY+1;
%             else
%                 j = j+1;
%             end
%         else
...
%         end
%     end
    %get the max and min coordinates for the longest possible meteor (x
    %direction)
    if numel(possibleMeteors) > 0
        [~,possibleMeteorIndex] = max(possibleMeteors(:,1));
        if abs(thetaMax) >= 45
...
%     if numel(possibleMeteorsX) > 0
%         [~,possibleMeteorIndexX] = max(possibleMeteorsX(:,1));
%         xCoords(i) = possibleMeteorsX(possibleMeteorIndexX,3);
%         yCoords(i) = round(intercept + xCoords(i)*slope);
%     end
    %get the max and min coordinates for the longest possible meteor (y
    %direction)
%     if numel(possibleMeteorsY) > 0
%         [~,possibleMeteorIndexY] = max(possibleMeteorsY(:,1));
%         xCoords(i) = possibleMeteorsY(possibleMeteorIndexY,3);
%         yCoords(i) = round(intercept + xCoords(i)*slope);
%     end
end
...
for i = 1:numel(radarGoodIndeces)
   ...
   [~,maxPowerIndex] = max(wideBeamPower(:,radarGoodIndeces(i)));
...
for i = 1:numel(meteorTimes)
   [~,radarValidTimeIndeces(i)] = min(abs(wideBeamTime-meteorTimes(i)));
end
