...
for a = 1:numel(thetax)
    ...
    F_Narrow(a,:) = sum(exp(j*(k*(mtimesx(x,ultraAnglesX(a,:))+mtimesx(y,ultraAnglesY(a,:)))+repmat(narrow_phases,1,size(ultraAnglesX,1)))),1);
    F_Wide(a,:) = sum(exp(j*(k*(mtimesx(x,ultraAnglesX(a,:))+  mtimesx(y,ultraAnglesY(a,:)))+repmat(wide_phases,1,size(ultraAnglesX,1)))),1);
    F_Para(a,:) = sum(exp(j*(k*(mtimesx(x,ultraAnglesX(a,:))+  mtimesx(y,ultraAnglesY(a,:)))+repmat(para_phases,1,size(ultraAnglesX,1)))),1);
