...
function [data, rawFrameInd, tUTC] =...
       rawDMCreaderLorenzoLimonta(BigFN,xPix,yPix,xBin,yBin,FrameInd,playMovie,Clim,rawFrameRate,startUTC,EventXPix,EventYPix)
...
if nargin<11, EventXPix=0;end
if nargin<12, EventXPix=0; EventYPix=0; end
EventXPix;
EventYPix;
%%
   ...
   if strcmpi(startUTC,'auto') % grep XML
     ...
     while ~feof(fidxml)
      ...
      if ~isempty(dates) && all(~isnan(dates)), 
        %
        %   BEGINNING OF LORENZO MODIFICATION
        %   N.B. date is in European format, i.e. dd/mm/yy
        %   bc date is first of april, length(dates)=5 rather than 6
        %   i.e. rather than being dates=010414, dates=10414,
        %   hence dates is changed to have the zero at the beginning
        if length(dates)==5
            dates=['0',dates];
        end
        %
        %   Same Problem with TIMES, since it's being read as integer
        %   the zero at the beginning is discarded!!! Add the zero back in
        %
        if length(times)==5
            times=['0',times];
        end
...
if playMovie   % ~= 0
  doPlayMovie(data,SuperX,SuperY,nFrameExtract,playMovie,Clim,rawFrameInd,tUTC, EventXPix, EventYPix)
end %for
%% cleanup
if ~nargout, clear, end
end %function

function doPlayMovie(data,nRow,nCol,nFrameExtract,playMovie,Clim,rawFrameInd,tUTC, EventXPix, EventYPix)
%% setup plot
...
try
    for iFrame = 1:nFrameExtract
     ...
     titlestring = ['Raw Frame # ',int2str(rawFrameInd(iFrame)),...
           '  Relative Frame # ',int2str(iFrame),' X Pixel to look at ',int2str(EventXPix),' Y-pixel to look at ',int2str(EventYPix)];
     if ~isempty(tUTC)
     ...
     pause(playMovie)
     hold on
     scatter(EventXPix,EventYPix,1000,'r')
    end
catch ME
    display(iFrame)
    display(tUTC(iFrame))
